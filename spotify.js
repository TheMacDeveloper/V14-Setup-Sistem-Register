...
module.exports = {
    ...
    execute: async (client, message, args, zetsu_embed) => {
        ...
        if (member && member.presence && member.presence.activities && member.presence.activities.some(five => five.name == "Spotify" && five.type == ActivityType.Listening)) {
          ...
          return message.reply({files:[{name:"canvafy.png",attachment:spotify}],embeds: [zetsu_embed.setImage('attachment://canvafy.png')] });
        }else{ return message.reply({embeds: [zetsu_embed.setDescription(`> **Kullanıcı Spotify Üzerinde Şarkı Dinlemiyor!**`)] });}
